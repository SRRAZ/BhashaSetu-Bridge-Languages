...
interface DailyStreakDao {
    ...
    @Query("UPDATE daily_streaks SET currentStreak = :newStreak, lastCheckInDate = :checkInDate WHERE id = 1")
    ...
    @Query("UPDATE daily_streaks SET currentStreak = 1, lastCheckInDate = :checkInDate WHERE id = 1")
    suspend fun resetStreak(checkInDate: Date)

    @Query("SELECT EXISTS(SELECT 1 FROM daily_streaks WHERE id = 1 AND date(lastCheckInDate/1000, 'unixepoch') = date('now'))")
