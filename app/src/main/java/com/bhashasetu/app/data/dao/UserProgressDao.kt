...
interface UserProgressDao {
    ...
    @Query("DELETE FROM user_learning_progress WHERE wordId = :wordId")
    suspend fun deleteProgressForWord(wordId: Long)
    
    // Queries
    
    @Query("SELECT * FROM user_learning_progress WHERE wordId = :wordId")
    ...
    @Query("SELECT * FROM user_learning_progress ORDER BY lastAttemptAt DESC")
    ...
    @Query("SELECT COUNT(*) FROM user_learning_progress")
    fun getTotalProgressCount(): Flow<Int>
    
    // Statistics queries
    
    @Query("SELECT COUNT(*) FROM user_learning_progress WHERE proficiencyLevel >= 85")
    fun getMasteredWordCount(): Flow<Int>
    
    @Query("SELECT COUNT(*) FROM user_learning_progress WHERE proficiencyLevel > 0 AND proficiencyLevel < 85")
    fun getLearningWordCount(): Flow<Int>
    
    @Query("SELECT COUNT(*) FROM user_learning_progress WHERE totalAttempts > 0")
    fun getStartedWordCount(): Flow<Int>
    
    @Query("SELECT SUM(timeSpentMs) FROM user_learning_progress")
